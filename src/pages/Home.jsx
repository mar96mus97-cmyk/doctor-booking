    ...
    <div className="container mx-auto p-4 max-w-5xl">
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {['اختر اليوم', 'اختر الوقت', 'أدخل بياناتك', 'تأكيد الحجز'].map((step, i) => {
            const colors = ['blue', 'green', 'purple', 'orange']
            return (
              <div key={i} className={`text-center p-6 bg-gradient-to-b from-${colors[i]}-50 to-white rounded-2xl border border-${colors[i]}-100 hover:shadow-lg transition-all duration-300`}>
                <div className={`bg-gradient-to-br from-${colors[i]}-600 to-${colors[i]}-700 text-white w-16 h-16 rounded-2xl flex items-center justify-center text-2xl font-extrabold mx-auto mb-4 shadow-lg`}>
                  {i + 1}
                </div>
                <h4 className="font-bold text-lg text-gray-800 mb-2">{step}</h4>
                <p className="text-gray-600 text-sm">
                  {['تصفح الأيام المتاحة واختر ما يناسبك من التقويم', 'اختر من بين الأوقات المتاحة للدكتور', 'أكمل نموذج الحجز بمعلوماتك الشخصية', 'اضغط تأكيد ويتم حجز موعدك فوراً'][i]}
                </p>
              </div>
            )
          })}
      ...
      <div className="text-center mt-8 text-gray-500 text-sm pb-4">
        <p>© ٢٠٢٤ عيادتي - جميع الحقوق محفوظة</p>
      </div>
    </div>
  )
}
